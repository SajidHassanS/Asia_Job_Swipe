...
export const fetchCompanyProfile = createAsyncThunk<Company, string, { rejectValue: string }>(
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        return rejectWithValue(
          error.response.data.message || "An error occurred while fetching the company profile."
        );
      } else {
...
>(
  ...
  async ({ id, updates, token }, { rejectWithValue }) => {
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        return rejectWithValue(
          error.response.data.message || "An error occurred while updating the company profile."
        );
      } else {
        return rejectWithValue("An unknown error occurred");
      }
...
>(
  ...
  async ({ companyId, logo, token }, { rejectWithValue }) => {
    try {
      const response = await axios.patch(`${FILE_URL}/files/company-logo/${companyId}`, logo, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      if (axios.isAxiosError(error) && error.response) {
        return rejectWithValue(
          error.response.data.message || "An error occurred while adding/updating the company logo."
        );
      } else {
        return rejectWithValue("An unknown error occurred");
      }
...
>(
  ...
  async ({ companyId, images, token }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.patch(`${FILE_URL}/files/company-images/${companyId}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "multipart/form-data",
        },
      ...
      if (axios.isAxiosError(error) && error.response) {
        return rejectWithValue(
          error.response.data.message || "An error occurred while adding/updating the company images."
        );
      } else {
        return rejectWithValue("An unknown error occurred");
      }
...
>(
  ...
  async ({ filename, token }, { rejectWithValue }) => {
    try {
      const response = await axios.delete(`${FILE_URL}/files/company-images/${filename}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
const companyProfileSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchCompanyProfile.fulfilled, (state, action: PayloadAction<Company>) => {
      ...
      .addCase(updateCompanyProfile.fulfilled, (state, action: PayloadAction<Company>) => {
        state.status = "succeeded";
        state.company = action.payload;
        state.error = null;
      })
