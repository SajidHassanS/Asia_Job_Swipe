...
export const getJobById = createAsyncThunk(
  ...
  async (jobId: string, { rejectWithValue }) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        return rejectWithValue(error.response.data.message || 'An error occurred while fetching the job.');
      } else if (error instanceof Error) {
        return rejectWithValue(error.message || 'An unknown error occurred');
      }
    }
...
export const updateJob = createAsyncThunk(
  ...
  async ({ id, jobData }: UpdateJobPayload, { rejectWithValue }) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        return rejectWithValue(error.response.data.message || 'An error occurred while updating the job.');
      } else if (error instanceof Error) {
        return rejectWithValue(error.message || 'An unknown error occurred');
      }
    }
...
export const deleteJob = createAsyncThunk(
  ...
  async (jobId: string, { rejectWithValue }) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        return rejectWithValue(error.response.data.message || 'An error occurred while deleting the job.');
      } else if (error instanceof Error) {
        return rejectWithValue(error.message || 'An unknown error occurred');
      }
    }
