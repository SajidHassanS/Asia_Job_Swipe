...
const SendOTPPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log('OAuth Callback Params:', { code, state }); // Debugging line

    if (code && state) {
      setRole(state);
      dispatch(googleSignIn({ code, role: state })).then((response) => {
        if (response.meta.requestStatus === 'fulfilled') {
          console.log('Google Sign-In Successful:', response.payload); // Debugging line
        } else {
          console.error('Google Sign-In Failed:', response.payload); // Debugging line
        }
      });
    }
  }, [dispatch]);
